...
void UInv_InventoryGrid::NativeTick(const FGeometry& MyGeometry, float InDeltaTime)
{
	...
	if (CursorExitedCanvas(CanvasPosition, UInv_WidgetUtils::GetWidgetSize(CanvasPanel), MousePositon))
	{
		return;
	}
...
void UInv_InventoryGrid::OnTileParametersUpdated(const FInv_TileParameters& Parameters)
{
	...
	{
		// There's a single item in the space, we can swap or add stacks;
...
FInv_SpaceQueryResult UInv_InventoryGrid::CheckHoverPosition(const FIntPoint& Position, const FIntPoint& Dimensions)
{
	...
	{
		int32 index = *OccupiedUpperLeftIndices.CreateConstIterator();
...
void UInv_InventoryGrid::ChangeHoverType(const int32 Index, const FIntPoint& Dimensions, EInv_GridSlotState GridSlotType)
{
	UnhighlightSlots(LastHighlightIndex, LastHighlightDimensions);

	UInv_InventoryStatics::ForEach2D(GridSlots, Index, Dimensions, Columns,
		[State = GridSlotType](UInv_GridSlot* GridSlot)
		{
			switch (State)
			{
...
void UInv_InventoryGrid::OnGridSlotHovered(int32 GridIndex, const FPointerEvent& MouseEvent)
{
	...
	UInv_GridSlot* GridSlot = GridSlots[GridIndex].Get();
	if (GridSlot->IsAvailable())
	{
		GridSlot->SetOccupiedTexture();
	}
...
void UInv_InventoryGrid::OnSlottedItemClicked(int32 GridIndex, const FPointerEvent& MouseEvent)
{
	...
	if (IsSameStackable(ClickedInventoryItem))
	{
		...
		// Should we fill in the stacks of the clicked item ? (and not consum the hover item, just decrease it's nums)
...
void UInv_InventoryGrid::SwapWithHoverItem(UInv_InventoryItem* ClickedInventoryItem, const int32 GridIndex)
{
	...
	AssignHoverItem(ClickedInventoryItem, GridIndex, HoverItem->GetPreviousGridIndex());	// 注册 HoverItem
	RemoveItemFromGrid(ClickedInventoryItem, GridIndex);									// 移除被点击 Item
...
bool UInv_InventoryGrid::ShouldFillInStack(const int32 RoomInClickedSlot, const int32 HoverStackCount)
{
